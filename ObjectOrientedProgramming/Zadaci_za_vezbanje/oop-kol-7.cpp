...
public:
    ...
    virtual char *getSmetka() {
        return this->smetka;
    }

    friend ostream &operator<<(ostream &out, Karticka &other) {
        out << other.smetka << ": " << other.tezinaProbivanje() << endl;
        return out;
    }
...
class SpecijalnaKarticka : public Karticka {
    int *additionalPins;
    int numberOfAdditionalPins;
    static int maxAmountOfPins;
public:
    SpecijalnaKarticka(char *smetka, int pin) : Karticka(smetka, pin) {
        this->additionalPins = new int[0];
        this->numberOfAdditionalPins = 0;
        this->povekjePin = true;
    }

    SpecijalnaKarticka(Karticka &other) : Karticka(other.getSmetka(), other.getPin()) {
        this->numberOfAdditionalPins = 0;
        this->additionalPins = new int[0];
        this->povekjePin = true;
    }
    ...
    SpecijalnaKarticka(SpecijalnaKarticka &other) : Karticka(other.smetka, other.pin) {
        this->numberOfAdditionalPins = other.numberOfAdditionalPins;
        this->additionalPins = new int[numberOfAdditionalPins];
        for (int i = 0; i < numberOfAdditionalPins; i++) {
            this->additionalPins[i] = other.additionalPins[i];
        }
    ...
    SpecijalnaKarticka &operator+=(int n) {
        if (numberOfAdditionalPins + 1 < maxAmountOfPins) {
            int *tmp = new int[numberOfAdditionalPins + 1];
            for (int i = 0; i < numberOfAdditionalPins; i++) {
                tmp[i] = additionalPins[i];
            }
            tmp[numberOfAdditionalPins++] = n;
            this->setPovekjePin(true);
    ...
    int tezinaProbivanje() {
        return Karticka::tezinaProbivanje() + numberOfAdditionalPins;

    }

};

int SpecijalnaKarticka::maxAmountOfPins = 4;
...
public:
    ...
    void pecatiKarticki() {
        ...
        for (int j = 0; j < broj; j++) {
            if (karticki[j]->tezinaProbivanje() <= getLIMIT()) {
                cout << *karticki[j];
            }
    ...
    void dodadiDopolnitelenPin(char *smetka, int novPin) {
        ...
        for (int i = 0; i < broj; i++) {
            bool check = strcmp(karticki[i]->getSmetka(), smetka);
            if (check == 0 && karticki[i]->isPovekjePin()) {
                SpecijalnaKarticka *sPtr = dynamic_cast<SpecijalnaKarticka *>(karticki[i]);
                if (sPtr != 0) {
                    SpecijalnaKarticka &sk = dynamic_cast<SpecijalnaKarticka &>(*karticki[i]);
                    sk += novPin;
                } else {
                    karticki[i] = new SpecijalnaKarticka(*karticki[i]);
                    karticki[i]->setPovekjePin(true);
                    SpecijalnaKarticka &sk = dynamic_cast<SpecijalnaKarticka &>(*karticki[i]);
                    sk += novPin;
                }
