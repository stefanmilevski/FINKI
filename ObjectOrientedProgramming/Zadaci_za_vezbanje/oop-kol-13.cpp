...
protected:
    ...
    int height;
public:
    Image(char *imgName = "untitled", char *owner = "unknown", int width = 800, int height = 800) {
    ...
    Image(Image &i) {
        if (this != &i) {
            this->imgName = new char[strlen(i.imgName) + 1];
            strcpy(this->imgName, i.imgName);
    ...
    friend ostream &operator<<(ostream &o, Image &i) {
        ...
        o << i.imgName << " " << i.owner << " " << i.fileSize() << endl;
        return o;
    }
...
public:
    ...
    TransparentImage(char *imgName = "untitled", char *owner = "unknown", int width = 800, int height = 800,
                     bool transparent = true) : Image(imgName, owner, width, height) {
        this->transparent = transparent;
    }

    TransparentImage(TransparentImage &other) : Image(other.imgName, other.owner, other.width, other.height) {
...
private:
    ...
    int numberOfImages;
public:
    Folder(char *folderName = "", char *folderOwner = "unknown", int numberOfImages = 0) {
    ...
    Folder operator+=(Image &img) {
        Image **tmp = new Image *[numberOfImages + 1];
        for (int i = 0; i < numberOfImages; i++) {
            tmp[i] = images[i];
        }
        tmp[numberOfImages++] = &img;
        delete[] images;
        images = tmp;
        return *this;
    }
    ...
    Image *getMaxFile() {
        ...
        int maxIndex = -1;
        for (int i = 0; i < numberOfImages; i++) {
            if (images[i]->fileSize() > maxSize) {
                maxSize = images[i]->fileSize();
                maxIndex = i;
            }
        }
    ...
    friend ostream &operator<<(ostream &out, Folder &f) {
        out << f.folderName << " " << f.folderOwner << endl;
        out << "--" << endl;
        for (int i = 0; i < f.numberOfImages; i++) {
...
Folder &max_folder_size(Folder *folders, int n) {
    ...
    int maxIndex = -1;
    for (int i = 0; i < n; i++) {
        if (folders[i].folderSize() > maxSize) {
            maxSize = folders[i].folderSize();
            maxIndex = i;
        }
    }
    return folders[maxIndex];
