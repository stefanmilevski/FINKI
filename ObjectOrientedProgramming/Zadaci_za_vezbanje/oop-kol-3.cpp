...
public:
    ...
    virtual int getBodovi() {
        ...
        return countPassedExams * 100 / brojOcenki;
    }
...
public:
    Demonstrator(int indeks, int *ocenki, int brojOcenki, char *imeIPrezime, Kurs *kursevi, int brojKursevi,
                 int brojNaCasovi) : Student(indeks, ocenki, brojOcenki), Predavach(imeIPrezime, kursevi, brojKursevi) {
        this->brojNaCasovi = brojNaCasovi;
    }

    Demonstrator(Demonstrator &other) : Student(other), Predavach(other) {
        this->brojNaCasovi = other.brojNaCasovi;
    }
    ...
    int getBodovi() {
        ...
        int bodovi = (20 * brojNaCasovi) / getBrojKursevi();
        return Student::getBodovi() + bodovi;
    ...
    void pecati() {
        Student::pecati();
        cout << ": " << imeIPrezime << " (";
        for (int i = 0; i < this->getBrojKursevi(); i++) {
            cout << this->getKursevi()[i].getIme(); // addextra
            cout << " " << this->getKursevi()[i].getKrediti() << "ECTS";
            if (i + 1 < this->getBrojKursevi()) {
                cout << ", ";
            }
        }
        cout << ")" << endl;
...
Student &vratiNajdobroRangiran(Student **studenti, int n) {
    ...
    for (int i = 0; i < n; i++) {
        try {
            if (studenti[i]->getBodovi() > maxPoints) {
                maxPoints = studenti[i]->getBodovi();
                maxIndex = i;
            }
...
void pecatiDemonstratoriKurs(char *kurs, Student **studenti, int n) {
    for (int i = 0; i < n; i++) {
        Demonstrator *dPtr = dynamic_cast<Demonstrator *>(studenti[i]);
        if (dPtr != 0) {
            for (int j = 0; j < dPtr->getBrojKursevi(); j++) {
                if (dPtr->getKursevi()[j] == kurs) {
                    dPtr->pecati();
                }
            }
        }
