...
protected:
    ...
    float cenaDenari;
public:
    Koncert(char *naziv, char *lokacija, float cena) {
        strcpy(this->naziv, naziv);
        strcpy(this->lokacija, lokacija);
        this->cenaDenari = cena;
    }

    static void setSezonskiPopust(float n) {
        Koncert::sezonskiPopust = n;
    }
    ...
    virtual float cena() {
        return cenaDenari * (1 - sezonskiPopust);
    }
};
...
class ElektronskiKoncert : public Koncert {
    ...
    bool dnevna;
public:
    ElektronskiKoncert(char *naziv, char *lokacija, float cena, char *djIme, float vremeTraenje, bool dnevna) : Koncert(
            naziv, lokacija, cena) {
...
void najskapKoncert(Koncert **koncerti, int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        ElektronskiKoncert *ePtr = dynamic_cast<ElektronskiKoncert *>(koncerti[i]);
        if (ePtr != 0) {
            elektronski++;
        }
    }
...
bool prebarajKoncert(Koncert **koncerti, int n, char *naziv, bool elektronski) {
    for (int i = 0; i < n; i++) {
        if (elektronski) {
            ElektronskiKoncert *ePtr = dynamic_cast<ElektronskiKoncert *>(koncerti[i]);
            if (ePtr != 0) {
                if ((strcmp(ePtr->getNaziv(), naziv) == 0)) {
                    cout << ePtr->getNaziv() << " " << ePtr->cena() << endl;
                    return true;
                }
            }
        } else {
