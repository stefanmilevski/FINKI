...
public:
    ...
    void izvestajVoEvra() {
        ...
        double amountInEur = total / Transakcija::getEUR();
        for (int i = 0; i < numberOfTransactions; i++) {
            amountInEur += transactions[i]->voEvra();
        }

        cout << "Korisnikot so smetka: " << accountNumber << " ima momentalno saldo od " << amountInEur << " EUR"
             << endl;
    }

    void izvestajVoDolari() {

        double amountInUSD = total / transactions[0]->getUSD();
        for (int i = 0; i < numberOfTransactions; i++) {
            amountInUSD += transactions[i]->voDolari();
        }

        cout << "Korisnikot so smetka: " << accountNumber << " ima momentalno saldo od " << amountInUSD << " USD"
             << endl;
    }
